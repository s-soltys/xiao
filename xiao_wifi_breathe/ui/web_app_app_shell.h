...
const char kWebAppAppShell[] PROGMEM = R"HTML(
      function AppShell() {
        ...
        useEffect(() => {
          if (activeApp !== 'wifi' && activeApp !== 'device') {
            lastDashboardAppRef.current = activeApp;
          }
        }, [activeApp]);
        ...
        const focusedApp = activeApp === 'wifi' || activeApp === 'device';
              ...
              <div className="flex flex-col gap-8">
                {focusedApp ? (
                  <section className="flex items-center justify-start">
                    <button
                      type="button"
                      onClick={() => setActiveApp(lastDashboardAppRef.current || 'matrix')}
